! function (e) {
  ...
  e.exports = class {
    constructor({
      ...
      this.type = e, this.orderForm = "", this.orderId = this.orderForm ? this.orderForm.orderFormId : "", this.lang = "", this.accordionPayments = t, this.deliveryDateFormat = a, this.quantityPriceCart = i, this.showNoteField = n, this.hideEmailStep = o
    }
    general() {
        ...
        $.each(e.items, (function (e) {
          ...
          if (t.assemblies.length > 0) {
            ...
            $.each(t.assemblies, (function (e) {
              let t = this.inputValues;
              a += `<p>${this.id}</p>`, a += '<ul class="v-custom-assemblies__values">', Object.entries(t).forEach(([e, t]) => {
    ...
    showCustomMsgCoupon(e) {
      let t = e.marketingData.coupon,
        a = e.items.reduce((function (e, a) {
          return e + (a.priceTags.length ? a.priceTags.filter(e => e.ratesAndBenefitsIdentifier ? e.ratesAndBenefitsIdentifier.matchedParameters["couponCode@Marketing"] == t : 0).length : 0)
        }), 0);
      if (!t || a > 0) return $("fieldset.coupon-fieldset").removeClass("js-vcustom-showCustomMsgCoupon"), $(".vcustom-showCustomMsgCoupon").remove(), !1;
    ...
    addLabels(e) {
      ...
      try {
        $(".table.cart-items tbody tr.product-item, .mini-cart .cart-items li").removeClass("v-custom-addLabels-active js-vcustom-addLabels"), $(".v-custom-addLabels-active-flag").remove(), $.each(e.items, (function (e) {
          this.priceTags.length > 0 && this.priceTags.filter(e => !!e.ratesAndBenefitsIdentifier && e.ratesAndBenefitsIdentifier.matchedParameters["couponCode@Marketing"] == t).length > 0 && (a.push(this), $(`.table.cart-items tbody tr.product-item:eq(${e})`).addClass("v-custom-addLabels-active js-vcustom-addLabels").find(".product-name").append(`<span class="v-custom-addLabels-active-flag">${t}</span>`))
        }))
      } catch (e) {
        console.error(e)
      }
    ...
    indexedInItems(e) {
      ...
      try {
        ...
        if (e.items) {
          let s = e.items.reduce((e, t) => (null != t.parentItemIndex && (e[t.parentItemIndex] = e[t.parentItemIndex] || [], e[t.parentItemIndex].push(t)), e), {});
          for (var a in s) {
            var i = s[a];
    ...
    addBusinessDays(e, t = i18n.options.lng) {
      ...
      return "pt" == t && (n = {
        ...
        month: "short",
        day: "numeric"
      }), a.getDate() - (new Date).getDate() == 1 ? this.lang.tomorrowLabel || "Tomorrow" : (a = a.toLocaleDateString(t, n), a)
    }
    changeShippingTimeInfo() {
      let e = this;
      $("body").addClass("v-custom-changeShippingTimeInfo");
      let t = [".shp-summary-package-time > span", "p.vtex-omnishipping-1-x-sla.sla", ".vtex-omnishipping-1-x-leanShippingTextLabelSingle > span", "span.shipping-date", ".shp-option-text-time", ".pkpmodal-pickup-point-sla", ".shp-option-text-package", ".srp-delivery-current-many__sla"];
      try {
        $("\n        .vtex-omnishipping-1-x-summaryPackage.shp-summary-package:not(.v-changeShippingTimeInfo-active), \n        .vtex-omnishipping-1-x-leanShippingOption, \n        .vtex-omnishipping-1-x-packageItem:not(.v-changeShippingTimeInfo-active),\n        .orderform-template .cart-template.mini-cart .item,\n        .vtex-pickup-points-modal-3-x-pickupPointSlaAvailability,\n        .srp-delivery-current-many\n      ").each((function (a) {
          let i = $(this).find(t.map(e => e + ":not(.v-changeShippingTimeInfo-elem-active)").join(", ")).text();
          if ("" != i && i.match(/(day)|(dia)/gm)) {
            let a = parseInt(i.match(/\d+/));
            if (a) {
              let i = e.lang.deliveryDateText;
              $(this).find(t.join(", ")).text().toLowerCase().match(/(ready in up)|(pronto)/gm) && (i = e.lang.PickupDateText), $(this).find(t.join(", ")).html(`${i} <strong>${e.addBusinessDays(a)}</strong>`).addClass("v-changeShippingTimeInfo-elem-active")
            }
          }
          $(this).addClass("v-changeShippingTimeInfo-active")
        }));
        let n = $(".srp-delivery-info .srp-packages:not(.v-changeShippingTimeInfo-elem-active)");
        if ($(".js-shippingPreviewPackges").remove(), n.length) {
          var a = n.text().split(":")[1].split(/,| and | e /),
            i = [];
          $.each(a, (function (t) {
            let n = a[t];
            if ("" != n && n.match(/(day)|(dia)/gm)) {
              let t = parseInt(n.match(/\d+/));
              if (t) {
                let a = e.lang.deliveryDateText;
                n.toLowerCase().match(/(ready in up)|(pronto)/gm) && (a = e.lang.PickupDateText), i.push(`${a} <strong>${e.addBusinessDays(t)}</strong>`)
              }
            }
          })), n.hide().after(`<p class="black-50 mt3 mb0 js-shippingPreviewPackges">${n.text().split(":")[0]}: ${i.join("; ")}</p>`).addClass("v-changeShippingTimeInfo-active")
        }
      } catch (e) {}
    }
    ...
    enchancementTotalPrice(e) {
      let t = this;
      if (t.quantityPriceCart) try {
        $.each(e.items, (function (a) {
          let i = $(`.table.cart-items tbody tr.product-item:eq(${a})`);
          if (1 == this.quantity || 0 == i.find("td.product-price").find(".best-price").length) return;
          let n = i.find(".total-selling-price").text(),
          ...
          i.find("td.product-price").find(".vqc-ldelem").remove(), i.find("td.product-price").addClass("v-custom-quantity-price-active").prepend(o).append(`<div class="v-custom-quantity-price vqc-ldelem"><span class="v-custom-quantity-price__best">${n}</span></div>`), i.find("td.product-price").find("> .best-price").wrap('<div class="v-custom-quantity-price__list--selling"></div>'), i.find("td.product-price").find(".v-custom-quantity-price__list--selling").append(`<span class="vqc-ldelem"> ${t.lang.eachLabel}</span>`)
        }))
      } catch (e) {}
    }
    condensedTaxes(e) {
      let t = e.totalizers.filter(e => "CustomTax" == e.id);
      ...
      let a = `\n      <div class="vcustom-customTax-resume">\n       ${t.map(t=>`<p class="vcustom-customTax-resume__i"><span class="n">${t.name}</span><span class="v">${e.storePreferencesData.currencySymbol} ${(t.value/100).toFixed(2)}</span></p>`).join("")}\n      </div>\n    `,
        i = $("tr.CustomTax.CustomTax--total");
      i.length && (i.find(".vcustom-customTax-tot").remove(), i.find(".info").append(`<div class="vcustom-customTax-tot"><span>?</span> ${a}</div>`))
    }
    update(e) {
      let t = this;
      this.checkEmpty(e.items), this.addAssemblies(e), this.enchancementTotalPrice(e), this.bundleItems(e), this.buildMiniCart(e), this.condensedTaxes(e), this.setParentIndex(e), this.indexedInItems(e), n((function () {
    ...
    paymentBuilder(e) {
      ...
      if (e && 0 == $(".payment-group-item-cards").length && e.paymentData) {
        let a = ["debitCardPaymentGroup", "creditCardPaymentGroup"],
          i = "";
        $.each(a, (function (n) {
          i = '<span class="payment-group-item-cards">', $.each(e.paymentData.paymentSystems.filter(e => e.groupName == a[n]), (function (e) {
            i += `<span class="card-flag ${this.name}">${this.name}</span>`
          })), i += "</span>", t.accordionPayments && $("#payment-group-" + a[n]).append(i)
    ...
    bind() {
      ...
      })), $("body").on("click", ".vtex-pickup-points-modal-3-x-pickupDetailsHeaderButton, #map-canvas img, .vtex-omnishipping-1-x-pickupPointChange, .pkpmodal-pickup-point, .vtex-pickup-points-modal-3-x-modalDetailsBackLnk", (function (t) {
        setTimeout(() => {
    ...
    start() {
      ...
      try {
        $((function () {
          ...
          e.updateStep(), e.changeShippingTimeInfoInit(), e.checkProfileFocus(), e.orderForm && (e.buildMiniCart(e.orderForm), e.indexedInItems(e.orderForm), e.updateLang(e.orderForm), e.paymentBuilder(e.orderForm))
        })), $(window).on("orderFormUpdated.vtex", (function (t, a) {
          e.update(a)
        })), $(window).load((function () {
          e.builder(), e.checkProfileFocus(), e.changeShippingTimeInfoInit(), e.indexedInItems(vtexjs.checkout.orderForm)
...
$(document).ready(function () {
      ...
      if (window.location.href.includes("/profile")){
        ...
        }); $("input#cart-coupon.coupon-value.input-small").text("placeholder", ""); $("input#cart-coupon.coupon-value.input-small").text("placeholder", "Ejemplo: GH0987");
    ...
    var oldHref = document.location.ref; window.dataLayer = window.dataLayer || [];
      ...
      var observer = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
          if (oldHref != document.location.href) {
            ...
            var words = document.location.hash.split("/");
            if (!document.location.hash.includes("cart")) {
              push({
                event: "pageViewVirtual",
                location: window.location.href,
                page: window.location.pathname + window.location.hash,
                title: "Checkout - " + words[words.length - 1],
              });
            }
          }
        });
